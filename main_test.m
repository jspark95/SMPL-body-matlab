...
%% Arguments Setting
maxIter = 30;
maxJointAnglePenalty = pi/2;
maxShoulderAnglePenalty = pi/8;
rigidMachingRatio = 0.7;
rigidRatioIncrement = 0.05;
...
Wreg = 50.0;
[theta, ~, ~, ~, ~] = lsqnonlin(@(theta)EfuncOptimThetaLand(C, v_shaped_sampled, J, theta, s_lm(8:22), points(t_lm(8:22), :), lm_tree, Wreg, maxJointAnglePenalty), theta, [], [], opts);
...
for iter = 1:maxIter
    ...
    [Tform, ~] = pcregrigid(Src, Tar, 'Extrapolate', true, 'InlierRatio', rigidMachingRatio, 'Metric', 'pointToPlane', 'MaxIterations', 50);
        ...
        for y = 1:yGrid
            for z = 1:zGrid
                ...
                while (isempty(gridGroup{x, y, z}))
                    ...
                    gridGroup{x, y, z} = 1:nOfSrcSampling;
                    gridGroup{x, y, z} = gridGroup{x, y, z}(all(abs(srcVertices(gridGroup{x, y, z}, :) - [cenX cenY cenZ]) <= (gridStep/2) * cStep, 2));
                end
            end
        end
    end
    ...
    % Optimizing
    VselGrid = max(min(floor((tarVertices - minGrid) ./ gridStep) +1, [xGrid, yGrid, zGrid]), [1, 1, 1]);
    ...
    [beta, ~, ~, ~, ~] = lsqnonlin(@(beta)EfuncOptimBeta(C, beta, theta, gridGroup, VselGrid, rndSrcIdx, s_lm(8:22), tarVertices, points(t_lm(8:22), :), Wland, Wreg), beta, [], [], opts);
    ...
    [theta, ~, ~, ~, ~] = lsqnonlin(@(theta)EfuncOptimTheta(C, theta, v_shaped, J, gridGroup, VselGrid, rndSrcIdx, s_lm(8:22), tarVertices, points(t_lm(8:22), :), Wland, Wreg, maxJointAnglePenalty, maxShoulderAnglePenalty), theta, [], [], opts);
    ...
    rigidMachingRatio = rigidMachingRatio + rigidRatioIncrement;
    rigidMachingRatio = min (1.0, rigidMachingRatio);
end
...
function F = EfuncOptimThetaLand(Core, v_shaped_sampled, J, theta, srcLandIdx, tarLandVert, lm_tree, Wreg, maxJointAnglePenalty)
    ...
    R = max(sqrt(sum(theta.^2, 2)) - maxJointAnglePenalty, 0) ./ sqrt(length(theta));
...
function F = EfuncOptimTheta(Core, theta, v_shaped, J, gridGroup, VselGrid, srcICPIdx, srcLandIdx, tarVerts, tarLandVert, Wland, Wreg, maxJointAnglePenalty, maxShoulderAnglePenalty)
    % ICP Term
    VsrcICP = genPosedShape(Core, v_shaped(srcICPIdx, :), J, theta, srcICPIdx); % Vsrc_Trans
    ...
    % Regularization Term
    theta_s = [theta(1:13,:);theta(16,:);theta(19:24,:)];
%     R1 = max(sqrt(sum(theta_h.^2, 2)) - maxShoulderAnglePenalty, 0) ./ sqrt(length(theta_h));
    R2 = max(sqrt(sum(theta_s.^2, 2)) - maxJointAnglePenalty, 0) ./ sqrt(length(theta_s));
    F = [E_ICP(:); sqrt(Wland) .* E_land(:); sqrt(Wreg) .* R2(:)];
end
