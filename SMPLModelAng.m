function [v, v_shaped, v_posed, rot_J_global] = SMPLModelAng(Core, beta, theta)
...
rot_J(:, :, 1) = rodangle(theta(1, :), J(1,:));
for j = 2:length(Core.kinTree)
    childRot = rodangle(theta(j, :), (J(j, :) - J((Core.kinTree(1, j)+1), :)));
